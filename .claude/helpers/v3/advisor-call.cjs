...
if (!taskDescription && !userMessage) {
  console.error(
    JSON.stringify({
      error:
        'Usage: advisor-call.cjs --agent <name> --task "<description>" [--context "<findings>"] [--message "<user request>"]',
      exit_code: 1,
    })
  );
  process.exit(1);
...
if (availableProviders.length === 0) {
  console.error(
    JSON.stringify({
      error:
        'No advisor provider available. Set one of: OPENROUTER_API_KEY, ANTHROPIC_API_KEY, OLLAMA_HOST',
      exit_code: 4,
    })
  );
  process.exit(4);
...
if (advisorConfig.provider) {
  selectedProvider = availableProviders.find(
    (p) => p.name === advisorConfig.provider
  );
}
...
const DOMAIN_PROMPTS = {
  'test-generation':
    'You are the advisor for a test-generation executor. Respond in under 100 words, enumerated steps. Name concrete classes, methods, dependencies. Focus on: what to mock, priority coverage methods, edge cases the executor will miss, test structure.',
  'security-compliance':
    'You are the advisor for a security executor. Respond in under 100 words, enumerated steps. Name concrete attack vectors, CWE IDs, code locations. Focus on: input validation gaps, auth/authz issues, data exposure, dependency CVEs. Prioritize by exploitability.',
  'coverage-analysis':
    'You are the advisor for a coverage analysis executor. Respond in under 100 words, enumerated steps. Name concrete uncovered files, functions, branches. Focus on: highest business risk gaps, missed branch types, quick wins.',
  'cross-domain':
    'You are the advisor for a fleet commander. Respond in under 100 words, enumerated steps. Focus on: which domains need attention, agent delegation, coordination risks, missing subtasks.',
};
...
const cliArgs = aqeBin === 'npx' ? ['aqe', 'llm', 'advise'] : ['llm', 'advise'];
...
try {
  ...
  // Clean up transcript
  try {
    require('fs').unlinkSync(transcriptPath);
  } catch {}
