...
async function parallelAudit(urls) {
  ...
  for (let i = 0; i < urls.length; i += MAX_CONCURRENT) {
    ...
    console.log(
      `Auditing batch ${Math.floor(i / MAX_CONCURRENT) + 1}: ${chunk.length} URLs`
    );
...
crawlAndAudit(startUrl, parseInt(maxPages) || 50).then((r) =>
...
(async () => {
  console.log(
    '=== MULTI-TOOL ACCESSIBILITY SCAN (v7.0 PARALLEL + RESILIENT) ==='
  );
  ...
  if (axeResult.status === 'fulfilled') {
    ...
    results.byTool['axe-core'] = {
      success: false,
      error: axeResult.reason.message,
    };
  ...
  if (pa11yResult.status === 'fulfilled') {
    ...
    results.byTool['pa11y'] = {
      success: false,
      error: pa11yResult.reason.message,
    };
...
3. **GENERATE SPECIFIC FIX**:
   ...
   <button aria-label="[DESCRIPTION]">
     <!-- THIS (context-aware) -->
     <button aria-label="Add Adidas Superstar to cart - $99"></button>
   </button>
   ```
...
<!-- RATIONALE -->
- Visible label aids all users - aria-describedby provides additional context -
autocomplete enables autofill - inputmode shows numeric keyboard on mobile -
...
<!-- RATIONALE -->
- aria-label describes action, not icon - aria-expanded communicates state -
aria-controls links to menu element - SVG hidden from assistive tech
(decorative)
```
...
|----------|---------|----------|-------| | #767676 | #757575 | #595959 | Gray
text | | #0066cc | #0055b3 | #003d82 | Link blue | | #cc0000 | #b30000 | #8b0000
| Error red |
```
...
h1: Site Name - Main Page Title ├── h2: Products │ ├── h3: Shoes │ └── h3:
Clothing └── h2: Contact
```
...
<!-- BEFORE (inaccessible) -->
<div class="dropdown" onclick="toggleMenu()">Menu</div>
...
<iframe
  src="map.html"
  title="Store location map showing 5 nearby stores"
></iframe>
<iframe
  src="video.html"
  title="Product demonstration video with captions"
></iframe>
...
const imageData = fs
  .readFileSync('/tmp/a11y-work/frames/frame_01.jpg')
  .toString('base64');
  ...
  messages: [
    {
      ...
      content: [
        ...
        {
          type: 'text',
          text: 'Describe this video frame for accessibility captions.',
        },
      ],
    },
  ],
...
mcp__claude -
  flow_alpha__hooks_intelligence_pattern_search({
...
mcp__claude -
  flow_alpha__hooks_intelligence_pattern_store({
