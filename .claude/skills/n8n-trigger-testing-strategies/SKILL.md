...
async function testWebhook(webhookUrl: string): Promise<WebhookTestResult> {
  const testPayloads = [
    ...
    {
      type: 'large',
      data: { items: Array(1000).fill({ id: 1, name: 'test' }) },
    },
    // Nested data
    {
      type: 'nested',
...
async function testWebhookAuth(
  ...
  const scenarios = [
    ...
    {
      name: 'expired-auth',
      headers: { Authorization: `Bearer ${authConfig.expiredToken}` },
    },
  ];
  ...
  return {
    ...
    invalidRejected: !results.find((r) => r.scenario === 'invalid-auth')
      ?.authenticated,
    validAccepted: results.find((r) => r.scenario === 'valid-auth')
      ?.authenticated,
    results,
  };
}
...
async function testScheduleReliability(
  triggerId: string,
  testDuration: number
...
async function testPollingTrigger(
  triggerId: string,
  testConfig: PollingTestConfig
...
function checkDeduplication(executions: Execution[]): boolean {
  ...
  for (const exec of executions) {
    const itemIds =
      exec.data?.resultData?.runData?.Trigger?.[0]?.data?.main?.[0]?.map(
        (item) => item.json?.id
      );
...
async function testEventTrigger(
  ...
  return {
    ...
    averageLatency: average(
      results.filter((r) => r.triggered).map((r) => r.latency)
    ),
    results,
  };
}
...
async function testTriggerResponses(
  webhookUrl: string
): Promise<ResponseTestResult> {
