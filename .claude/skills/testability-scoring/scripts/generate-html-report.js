...
function normalizeReportData(data) {
  ...
  if (!normalized.timestamp) {
    normalized.timestamp =
      data.metadata?.assessmentDate || new Date().toISOString();
  }

  // Normalize recommendations to ensure all have required fields
  if (Array.isArray(normalized.recommendations)) {
    normalized.recommendations = normalized.recommendations.map(
      (rec, index) => {
        ...
        return {
          ...
          recommendation:
            rec.recommendation || rec.text || 'No description provided',
...
const outputPath =
  args[1] || `tests/reports/testability-report-${Date.now()}.html`;
...
console.log(
  `✓ Overall score: ${reportData.overall}/100 (${getLetterGrade(reportData.overall)})`
);
