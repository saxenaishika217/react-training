...
async function navigateToPage(page) {
  ...
  try {
    ...
    await page
      ...
      .catch(() =>
        console.log('[NAV] Network not idle after 15s, continuing...')
      );
...
test.describe('Comprehensive Testability Analysis - Sauce Demo Shopify', () => {
    ...
    try {
      ...
      try {
        ...
        const products = await page
          .locator('[data-product], .product, .product-item')
          .count();
        interactions.push({ action: 'browse', steps: products > 0 ? 2 : 5 });

        // Test cart interaction
        const cartExists =
          (await page.locator('[data-cart], .cart, #cart').count()) > 0;
        interactions.push({ action: 'cart', steps: cartExists ? 2 : 4 });

        // Test checkout flow
        const checkoutExists =
          (await page.locator('[href*="checkout"], .checkout-button').count()) >
          0;
        interactions.push({
          action: 'checkout',
          steps: checkoutExists ? 3 : 6,
        });
      } catch (e) {
      ...
      const avgComplexity =
        interactions.reduce((sum, i) => sum + i.steps, 0) / interactions.length;
  ...
  test('4. Algorithmic Transparency Assessment', async ({ page }) => {
    try {
      ...
      const hasDataAttributes = await page.evaluate(() => {
        return (
          document.querySelectorAll('[data-product], [data-cart], [data-price]')
            .length > 0
        );
      });
      ...
      console.error(
        'Algorithmic Transparency assessment failed:',
        error.message
      );
      testabilityScores.principles.algorithmicTransparency = {
        score: 50,
  ...
  test('5. Explainability Assessment', async ({ page }) => {
    try {
      ...
      const hasHelpText =
        (await page.locator('[aria-label], [title], .help-text').count()) > 0;

      // Check for clear error messages
      const hasErrorHandling = await page.evaluate(() => {
        return (
          document.querySelectorAll('[role="alert"], .error, .message')
            .length >= 0
        );
      });
      ...
      const hasGuidance =
        (await page.locator('[data-tooltip], .tooltip').count()) > 0;
  ...
  test('10. Decomposability Assessment', async ({ page }) => {
    try {
      ...
      const hasModularStructure = await page.evaluate(() => {
        const hasComponents =
          document.querySelectorAll(
            '[data-component], [data-module], .component, .module'
          ).length > 0;
        const hasSections =
          document.querySelectorAll('section, [role="region"]').length > 0;
      ...
      const hasIsolatedFeatures = await page.evaluate(() => {
        ...
        const hasProduct =
          document.querySelector('[data-product], .product') !== null;
