...
async function scanForExposedCredentials(
  ...
  const sensitivePatterns = [
    ...
    {
      name: 'Client Secret',
      pattern: /client[_-]?secret["\s:=]+["']?([a-zA-Z0-9_-]{20,})["']?/gi,
    },
    {
      name: 'Refresh Token',
...
async function verifyCredentialEncryption(
  ...
  return {
    ...
    secure:
      encryptionChecks.isEncrypted && encryptionChecks.algorithm !== 'unknown',
...
async function testCredentialRotation(
  credentialId: string
...
async function testWebhookInputValidation(
  webhookUrl: string
  ...
  for (const test of maliciousPayloads) {
    try {
      ...
      results.push({
        ...
        sanitized: !responseBody.includes(
          test.payload.text || test.payload.data
        ),
        recommendation:
          response.status === 500
...
async function scanExpressionsForSecurity(
  workflowId: string
): Promise<ExpressionSecurityResult> {
  const workflow = await getWorkflow(workflowId);
  ...
  const dangerousPatterns = [
    ...
    { name: '__proto__', pattern: /__proto__/g, severity: 'HIGH' },
    {
      ...
      pattern: /constructor\.prototype/g,
      severity: 'HIGH',
    },
  ];
...
async function scanExecutionLogs(
  ...
  return {
    ...
    secure: findings.length === 0,
    recommendation:
      findings.length > 0 ? 'Enable credential masking in n8n settings' : null,
  };
}
```
...
async function checkErrorMessageSecurity(
  workflowId: string
    ...
    try {
      ...
      const sensitiveData = [
        ...
        { name: 'Stack trace', pattern: /at\s+\w+\s+\([^)]+\)/i },
        {
          name: 'Internal IP',
          pattern:
            /\b(?:10|172\.(?:1[6-9]|2[0-9]|3[01])|192\.168)\.\d+\.\d+\b/i,
        },
      ];
