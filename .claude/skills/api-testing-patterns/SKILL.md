...
describe('Validation', () => {
  ...
  it('validates ranges', async () => {
    expect(
      (await api.post('/orders', { productId: 'abc', quantity: -5 })).status
    ).toBe(400);
  });
});
```
...
it('prevents duplicates with idempotency key', async () => {
  ...
  const r1 = await api.post('/orders', data, {
    headers: { 'Idempotency-Key': key },
  });
  const r2 = await api.post('/orders', data, {
    headers: { 'Idempotency-Key': key },
  });
...
it('handles race condition on inventory', async () => {
  const promises = Array(10)
    .fill()
    .map(() => api.post('/orders', { productId: 'abc', quantity: 1 }));
