...
describe('Content-Based Router - Order Type', () => {
  it('routes standard orders to fulfillment queue', async () => {
    ...
    const routed = await broker.consume('orders.fulfillment', {
      timeout: 5000,
    });
  ...
  it('sends unrecognized order types to DLQ', async () => {
    ...
    expect(dlqMessage.headers['x-error-reason']).toContain(
      'Unrecognized orderType'
    );
  });
});
```
...
describe('Header-Based Router - Region', () => {
  it('routes by x-region header to regional queues', async () => {
    ...
    for (const region of regions) {
      ...
      const routed = await broker.consume(`orders.${region.toLowerCase()}`, {
        timeout: 5000,
      });
  ...
  it('routes messages without region header to default queue', async () => {
    await broker.publish('orders.global', {
      body: { orderId: 'ORD-NO-REGION' },
    });
...
describe('Recipient List - Order Notifications', () => {
  it('fans out order to all registered recipients', async () => {
    const message = {
      body: { orderId: 'ORD-100', total: 250.0 },
    };
...
describe('JSON to XML Transformation', () => {
  it('transforms order JSON to SAP XML format', async () => {
    const jsonInput = {
      ...
      items: [{ sku: 'MAT-100', quantity: 10, unitPrice: 25.0 }],
    };

    await broker.publish('transform.json-in', { body: jsonInput });

    const xmlOutput = await broker.consume('transform.xml-out', {
      timeout: 5000,
    });
...
describe('Flat-File to XML Transformation', () => {
  it('parses fixed-width flat file into structured XML', async () => {
    ...
    const xmlOutput = await broker.consume('transform.flatfile-xml-out', {
      timeout: 5000,
    });
    const parsed = parseXml(xmlOutput.body);
...
describe('Message Ordering Validation', () => {
  it('preserves FIFO order within same partition key', async () => {
    const messages = Array.from({ length: 10 }, (_, i) => ({
      headers: {
        'x-partition-key': 'ORDER-GROUP-1',
        'x-sequence': String(i + 1),
      },
    ...
    const received = [];
    for (let i = 0; i < 10; i++) {
      const msg = await broker.consume('orders.sequenced.out', {
        timeout: 5000,
      });
  ...
  it('detects out-of-order messages via sequence gap', async () => {
    ...
    const alert = await broker.consume('orders.sequence-gap', {
      timeout: 10000,
    });
...
describe('Compensation Pattern - Saga', () => {
  it('compensates completed steps when a later step fails', async () => {
    ...
    await broker.publish('saga.order', {
      body: { orderId: 'ORD-SAGA', amount: 100.0 },
    });

    // Verify compensation messages were sent
    const paymentRefund = await broker.consume('saga.compensate.payment', {
      timeout: 10000,
    });
    expect(paymentRefund.body.action).toBe('REFUND');
    expect(paymentRefund.body.orderId).toBe('ORD-SAGA');

    const inventoryRelease = await broker.consume('saga.compensate.inventory', {
      timeout: 10000,
    });
...
describe('SOAP to REST Protocol Mediation', () => {
  it('converts SOAP request to REST call and back', async () => {
    ...
    const response = await soapClient.send(soapRequest, {
      endpoint: esbSoapEndpoint,
    });
...
describe('Splitter - Batch Order to Individual Items', () => {
  it('splits batch order into individual item messages', async () => {
    ...
    for (let i = 0; i < 3; i++) {
      splitMessages.push(
        await broker.consume('orders.items', { timeout: 5000 })
      );
    }

    expect(splitMessages).toHaveLength(3);
    expect(splitMessages.map((m) => m.body.sku).sort()).toEqual([
      'A1',
      'B2',
      'C3',
    ]);
...
describe('Aggregator - Collect Item Results', () => {
  it('aggregates individual item results into batch response', async () => {
    ...
    const aggregated = await broker.consume('orders.batch.completed', {
      timeout: 10000,
    });

    expect(aggregated.body.batchId).toBe('BATCH-002');
    expect(aggregated.body.items).toHaveLength(3);
    expect(aggregated.body.items.every((i) => i.status === 'PICKED')).toBe(
      true
    );
  });
  ...
  it('handles aggregation timeout for incomplete batches', async () => {
    ...
    const partial = await broker.consume('orders.batch.incomplete', {
      timeout: 60000,
    });
...
describe('Circuit Breaker', () => {
  it('opens circuit after threshold failures', async () => {
    ...
    for (let i = 0; i < 5; i++) {
      await broker.publish('orders.circuit', {
        body: { orderId: `ORD-CB-${i}` },
      });
    }
...
describe('Service Virtualization - Middleware Isolation', () => {
    ...
    virtualService = await mountebank.createImposter({
      ...
      stubs: [
        ...
        },
        {
          predicates: [
            {
              equals: { method: 'POST', path: '/sap/orders' },
  ...
  it('middleware correctly handles SAP success response', async () => {
    ...
    const result = await broker.consume('orders.sap-confirmed', {
      timeout: 5000,
    });
...
await Task(
  'Transformation Accuracy Suite',
  {
    transformations: [
      {
        name: 'JSON-to-SAP-XML',
        input: 'testdata/order.json',
        expected: 'testdata/order-sap.xml',
      },
      {
        name: 'Flat-to-XML',
        input: 'testdata/inventory.dat',
        expected: 'testdata/inventory.xml',
      },
    ],
...
await middlewareFleet.execute({
  flows: [
    {
      name: 'order-inbound',
      stages: ['route', 'transform', 'enrich', 'deliver'],
    },
