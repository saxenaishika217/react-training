...
dependencies:
  [api-testing-patterns, contract-testing, enterprise-integration-testing]
...
describe('FIFO Allocation', () => {
  it('allocates oldest lot first', async () => {
    ...
    await wms.receiveInventory('MAT-FIFO', 50, {
      lot: 'LOT-OLD',
  ...
  it('splits allocation across lots when oldest is insufficient', async () => {
    await wms.receiveInventory('MAT-SPLIT', 20, {
      lot: 'LOT-A',
      receiptDate: '2026-01-01',
    });
    await wms.receiveInventory('MAT-SPLIT', 50, {
      lot: 'LOT-B',
...
describe('FEFO Allocation (First Expired, First Out)', () => {
  it('allocates lot with earliest expiry first', async () => {
    await wms.receiveInventory('MAT-FEFO', 50, {
      lot: 'LOT-EXP-LATE',
      expiry: '2027-12-31',
    });
    await wms.receiveInventory('MAT-FEFO', 50, {
  ...
  it('excludes expired lots from allocation', async () => {
    await wms.receiveInventory('MAT-EXPIRED', 100, {
      lot: 'LOT-PAST',
      expiry: '2025-12-31',
    });
    await wms.receiveInventory('MAT-EXPIRED', 50, {
      lot: 'LOT-VALID',
      expiry: '2027-12-31',
    });
...
describe('Pack Verification', () => {
  ...
  it('rejects pack with missing items', async () => {
    ...
    expect(packResult.missingItems).toContainEqual({
      item: 'MAT-200',
      expectedQty: 5,
    });
  });
...
describe('WMS-ERP Goods Receipt Sync', () => {
  ...
  it('handles sync failure with IDoc error and retry', async () => {
    ...
    const retried = await sap.waitForIdocStatus(idoc.id, '53', {
      timeout: 60000,
    });
...
describe('Replenishment Triggers', () => {
  ...
  it('triggers demand-based replenishment from wave allocation', async () => {
    ...
    const wave = await wms.planWave({
      ...
      orders: [
        { orderId: 'SO-REPLEN', lines: [{ item: 'MAT-200', qty: 100 }] },
      ],
    });
