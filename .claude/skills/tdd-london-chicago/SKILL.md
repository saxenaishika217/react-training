...
describe('Order', () => {
  it('calculates total with tax', () => {
    ...
    order.addItem(new Product('Widget', 10.0), 2);
    order.addItem(new Product('Gadget', 15.0), 1);

    expect(order.totalWithTax(0.1)).toBe(38.5);
  });
});
```
...
describe('Order', () => {
  it('delegates tax calculation', () => {
    const taxCalculator = {
      calculateTax: jest.fn().mockReturnValue(3.5),
    };
    ...
    expect(taxCalculator.calculateTax).toHaveBeenCalledWith(20.0);
  });
});
```
