...
function captureScreenshot(selector, outputPath) {
  ...
  if (res.status !== 0) {
    throw new Error(
      `vibium screenshot failed: ${res.stderr.trim() || res.stdout.trim()}`
    );
  }
  const screenshotPath = candidates.find((candidate) =>
...
function compareFallback(baselineBuf, currentBuf) {
  ...
  return {
    ...
    note: same
      ? undefined
      : 'pixelmatch not installed; exact-hash fallback reports 0 similarity',
  };
}
...
function main() {
  ...
  const threshold =
    args.threshold !== undefined ? parseFloat(args.threshold) : 0.1;
