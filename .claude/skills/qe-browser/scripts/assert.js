...
function safeRegex(pattern) {
  ...
  if (!REGEX_PATTERN_ALLOWLIST.test(pattern)) {
    return {
      re: null,
      error:
        'pattern contains characters outside the allowlist (control chars, non-ASCII, etc.)',
    };
  }
...
function runConsoleCheck(kind, check) {
  ...
  if (kind === 'no_console_errors') {
    const errors = entries.filter((e) =>
      ['error', 'severe'].includes(
        String(e.level || e.type || '').toLowerCase()
      )
    );
  ...
  if (kind === 'console_message_matches') {
    ...
    const match = entries.find((e) =>
      re.test(String(e.message || e.text || ''))
    );
    return {
      ok: Boolean(match),
      actual: match ? match.message || match.text : null,
    };
  }
...
function runCheck(check) {
  ...
  let result;
  if (
    check.kind === 'no_console_errors' ||
    check.kind === 'console_message_matches'
  ) {
