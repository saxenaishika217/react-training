...
const PATTERNS = [
  ...
  },
  {
    ...
    severity: 'high',
    regex:
      /(enable|activate|enter)\s+(developer|dev|debug|jailbreak|admin|root)\s+mode/i,
  ...
  },
  {
    ...
    severity: 'medium',
    regex:
      /decode\s+(the\s+)?(following\s+)?base64\s+(and\s+(run|execute|follow))?/i,
  ...
  },
  {
    ...
    severity: 'medium',
    regex:
      /(this\s+is\s+anthropic|i\s+am\s+a\s+trusted|authorized\s+by\s+the\s+developer)/i,
...
function fetchPageText(includeHidden, excludeSelector) {
  ...
  const excludeJson = excludeSelector
    ? JSON.stringify(excludeSelector)
    : 'null';
  const script = `
    (function() {
...
function main() {
  ...
  const excludeSelector =
    typeof args['exclude-selector'] === 'string'
      ? args['exclude-selector']
      : null;
  ...
  try {
    ...
    const severity =
      findings.length === 0 ? 'none' : aggregateSeverity(findings);
    const status =
