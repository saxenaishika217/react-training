  ...
  switch (a) {
    ...
    case 'type':
      ...
      if (typeof step.text !== 'string')
        return `step ${index} (${a}): "text" must be a string`;
      break;
    case 'press':
    ...
    case 'wait_selector':
      if (!step.selector)
        return `step ${index} (wait_selector): missing "selector"`;
      break;
    case 'storage_save':
...
function dispatch(step) {
  ...
  switch (a) {
    ...
    case 'press':
      ...
      return runVibium(
        target ? ['press', step.key, target] : ['press', step.key]
      );
    case 'wait_url':
    ...
    case 'wait_load':
      return runVibium(
        step.timeoutMs
          ? ['wait', 'load', '--timeout', String(step.timeoutMs)]
          : ['wait', 'load']
      );
    case 'map':
      return vibiumJson(
        step.selector ? ['map', '--selector', step.selector] : ['map']
      );
    case 'screenshot': {
    ...
    case 'assert': {
      ...
      if (res.status !== 0) {
        throw new Error(
          `assert step failed: ${res.stdout.trim() || res.stderr.trim()}`
        );
      }
