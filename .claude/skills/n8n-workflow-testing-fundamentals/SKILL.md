...
async function testErrorHandling(workflowId: string) {
  ...
  for (const scenario of errorScenarios) {
    ...
    const execution = await executeWithErrorInjection(
      workflowId,
      scenario.inject
    );
...
async function checkErrorWorkflowTriggered(
  executionId: string
): Promise<boolean> {
...
const workflowAssertions = {
  ...
  assertDataTransformed: (execution, nodeName, expectedData) => {
    const nodeOutput =
      execution.data.resultData.runData[nodeName][0].data.main[0][0].json;
